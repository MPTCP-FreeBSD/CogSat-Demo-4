function DeakinCogSatSimulatorV12
    clc
    ...
    GEOUsers=uidropdown(fig, 'Items', {'5','10','15'},'FontSize', 16, 'Position', [170 730 100 25], 'Value', '10');
    uilabel(fig, 'Text','LEO users:', 'FontSize', 18, 'Position', [40 690 180 25]);
    LEOUsers=uidropdown(fig, 'Items', {'5','10','15'},'FontSize', 16, 'Position', [170 690 100 25], 'Value', '10');
    ...
    setupKPIAxes(kpi2ax, [-1 6], 'Throughput [bps/Hz]','Time [s]',[]);
    ...
    setupKPIAxes(kpi5ax, [-25 15], 'SINR [dB]','User index',[]);
    % uiimage(kpi5, ...
    ...
    function T = loadDataForCombination()
        % Read current dropdown values
        selectedChannels = Channelnum.Value;
        selectedLEO = LEOUsers.Value;
        selectedGEO = GEOUsers.Value;
    
        % Construct filename using selected values
        filename = sprintf('merged_leo_geo_data_%s_channels_%s_leo_user_%s_geo_user.xlsx', ...
                            selectedChannels, selectedLEO, selectedGEO);
    
        fullpath = fullfile('Data', filename);  % Adjust folder if needed
        ...
        if isfile(fullpath)
            ...
            uialert(fig, ['File not found: ' filename], 'Missing File');
            T = table();  % Return empty table if not found
